 ...
 */
export function serverLog(level: 'INFO' | 'ERROR' | 'DEBUG' | 'WARN', message: string, data?: unknown): void {
    if (!headerEmitted) {
...
export function cleanStackTrace(stack: string): string {
    return stripAnsi(stack)
        ...
        .filter(line => {
            ...
            if (line.includes('internal/process')) return false;
            if (line.includes('Generator.next')) return false;
            if (line.includes('__awaiter')) return false;
 ...
 */
export function extractLogsForTest(logContent: string, testName: string): string[] {
    const logs: string[] = [];
